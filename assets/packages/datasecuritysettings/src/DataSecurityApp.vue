    ...
    export default {
        ...
        data() {
            return {
                ...
                legalNoticeMessageEditorObject: ClassicEditor,
                legalnoticemessageEditorData: {
                    'lsExtension:fieldtype': 'survey-legalnoticemessage',
                    'lsExtension:ajaxOptions': {surveyid: this.$store.getters.surveyid},
                    'lsExtension:currentFolder': 'upload/surveys/' + this.$store.getters.surveyid + '/'
                },
                event: null,
        ...
        computed: {
            ...
            currentShowDataSecurityButton: {
                get() {
                    return parseInt(this.$store.state.showdatasecuritybutton)
                },
                set(newValue) {
                    this.$store.commit('setShowDataSecurityButton', newValue);
                }
            },
            ...
            currentLegalNoticeMessage: {
                get() {
                    return this.$store.state.legalnoticemessage[this.$store.state.activeLanguage];
                },
                set(newValue) {
                    this.$store.commit('setLegalNoticeMessageForCurrentLanguage', newValue);
                }
            },
    ...
    <div class="container-center">
        <template v-if="!loading">
            <div class="data-policy-buttons">
                <div class="row">
                    <div class="col-sm-6">
                        <label class="control-label">{{"Show survey policy text with mandatory checkbox"|translate}}:</label>
                    ...
                    <div class="col-sm-6">
                        <div class="row">
                            <div class="col-sm-6">
                                <label class="control-label" for='showdatapolicybutton'>{{"Show data policy in survey"|translate}}:</label>
                                <bootstrap-toggle id="showdatapolicybutton" v-model="currentShowDataSecurityButton" :options="switchOptions" :disabled="disabled || readonly"/>
                            </div>
                            <div class="col-sm-6">
                                <label class="control-label" for='showlegalnoticebutton'>{{"Show legal notice in survey"|translate}}:</label>
                                <bootstrap-toggle id="showlegalnoticebutton" v-model="currentShowLegalNoticeButton" :options="switchOptions" :disabled="disabled || readonly"/>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div v-show="currentShowsurveypolicynotice > 0" class="container-center">
                        ...
                        <div class="ls-flex-row col-12">
                            ...
                            <div class="ls-flex-item text-right" v-if="$store.state.permissions.update">
                                <button class="btn btn-default btn-xs"
                                        @click.prevent="sourceEditEnable=!sourceEditEnable"><i
                                        class="fa fa-file-code-o"></i>{{'Toggle source mode'|translate}}
                                </button>
                            </div>
                        </div>
                        <div v-if="!$store.state.permissions.update" class="col-12"
                             v-html="stripScripts(currentDatasecmessage)"/>
                        <lsckeditor :editor="datasecmessageEditorObject"
                                    v-if="!sourceEditEnable && $store.state.permissions.update"
                                    v-model="currentDatasecmessage" :config="datasecmessageEditorData"></lsckeditor>
                        <aceeditor v-if="sourceEditEnable && $store.state.permissions.update"
                                   thisID="datasecmessageSourceEditor" v-model="currentDatasecmessage"
                                   :showLangSelector="false"></aceeditor>
                        <input v-if="$store.state.permissions.update" type="hidden" name="surveyls_policy_notice"
                               v-model="currentDatasecmessage"/>
                    </div>
                </div>
                <div class="row scoped-editor-row">
                    <div class="col-sm-12 ls-space margin top-5 bottom-5 scope-contains-ckeditor ">
                        <div class="ls-flex-row col-12">
                            <div class="ls-flex-item text-left">
                                <label class="">{{ "Survey legal notice" | translate }}:</label>
                            </div>
                            <div class="ls-flex-item text-right" v-if="$store.state.permissions.update">
                                <button class="btn btn-default btn-xs"
                                        @click.prevent="sourceEditEnable=!sourceEditEnable"><i
                        ...
                        <div v-if="!$store.state.permissions.update" class="col-12"
                             v-html="stripScripts(currentLegalNoticeMessage)"/>
                        <lsckeditor :editor="legalNoticeMessageEditorObject"
                                    v-if="!sourceEditEnable && $store.state.permissions.update"
                                    v-model="currentLegalNoticeMessage"
                                    :config="legalnoticemessageEditorData"></lsckeditor>
                        <aceeditor v-if="sourceEditEnable && $store.state.permissions.update"
                                   thisID="legalnoticeSourceEditor" v-model="currentLegalNoticeMessage"
                                   :showLangSelector="false"></aceeditor>
                        <input v-if="$store.state.permissions.update" type="hidden" name="surveyls_legal_notice"
                               v-model="currentLegalNoticeMessage"/>
                    </div>
                </div>
            </div>
        ...
        <div v-if="loading">
            <loader-widget id="textelementsloader"/>
        </div>
    </div>
</template>
